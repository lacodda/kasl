...
case "$(uname -s)-$(uname -m)" in
    ...
    MINGW*|MSYS*|CYGWIN*)
        ...
        ;;
    *)
        echo "No prebuilt binary for $(uname -s)/$(uname -m); install with: cargo install kasl-cli" >&2
        exit 1
        ;;
esac
...
case "$TAG" in
    v[0-9]*) ;;
    *)
        echo "Cannot resolve the latest release of $REPO - set KASL_VERSION to a release tag (vX.Y.Z)" >&2
        exit 1
        ;;
esac
...
# `ka` can never fall behind the version `kasl` was updated to. The archive
# carries one binary and no `ka` of its own.
...
rm -f "$BIN_DIR/kasl.bak" "$BIN_DIR/ka.bak"
