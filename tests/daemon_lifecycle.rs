...
mod tests {
    ...
    fn test_stop_with_nonexistent_process(ctx: &mut DaemonTestContext) {
        ...
        match result {
            Ok(_) => {
                // PID file may or may not be cleaned up depending on
                // the platform implementation; either outcome is fine here.
            }
            Err(_) => {
    ...
    fn test_process_termination_by_pid(_ctx: &mut DaemonTestContext) {
        ...
        #[cfg(windows)]
        {
            let mut child = Command::new("cmd").args(["/C", "ping 127.0.0.1 -n 2"]).spawn().unwrap();
