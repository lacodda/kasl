...
fn stop_internal() -> Result<(), Box<dyn Error>> {
    let pid_path = DataStorage::new().get_path(PID_FILE)?;
    if !pid_path.exists() {
    ...
    let pid: u32 = pid_str.trim().parse().map_err(|_| "Invalid PID file content")?;
    ...
    if killed {
        ...
        Err(format!("Failed to stop watcher process (PID: {})", pid).into())
    }
}
...
fn kill_process(pid: u32) -> Result<bool, Box<dyn Error>> {
    ...
    unsafe {
        ...
        if handle.is_null() {
            ...
            return Err(format!("Failed to open process: error code {}", error).into());
        }
        ...
        if result == 0 {
            let error = GetLastError();
            Err(format!("Failed to terminate process: error code {}", error).into())
        } else {
...
#[cfg(not(any(unix, windows)))]
fn kill_process(_pid: u32) -> Result<bool, Box<dyn Error>> {
